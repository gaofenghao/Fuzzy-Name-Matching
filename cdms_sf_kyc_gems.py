...
#cdms_unmatched = pd.read_excel('H:/Client Name Truth/Data/CDMS_TO_GEMS_MASTER.xlsx', sheetname = 'UNMATCHEDTOCHECK')
cdms_unmatched = pd.read_excel('H:/Client Name Truth/Data/cdms_unmatched.xlsx')
...
sf360 = pd.read_excel('H:/Client Name Truth/Data/cdms_to_SF360_Mapping.xlsx')
kycall = pd.read_excel('H:/Client Name Truth/Data/KYC Legal_Entity_Details.xlsx')
...
for j in range(len(cdms_unmatched)):
    ...
    if finalscore >=0.95:
        ...
        tmp2 = sf360[sf360['UP'] == cdms_unmatched['CDMS CODE'].values[j]]
        tmp2 = tmp2[tmp2['Level'] == 1]
        if len(tmp2) != 0:
            tmp3 = kycall['Legal Entity Name'].apply(partial(stringmatcher1,tmp2['SF360 Account Name'].values[0]))
            kscore = max(tmp3)
            if kscore >=0.9:
                kind = np.argmax(tmp3)
                ...
                cdmsnmsf = tmp2['SF360 Account Name'].values[0].lower()
                score1 = 0.0
                score2 =0.0
                score3 =0.0
...
cdms_save = pd.read_excel('H:/Client Name Truth/Data/CDMS_TO_GEMS_MASTER.xlsx', sheetname = 'UNMATCHEDTOCHECK')
